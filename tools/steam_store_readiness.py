...
def command_validate(args: argparse.Namespace) -> int:
    definition, state = load_pair(
        args,
        require_files=not args.skip_evidence_existence,
    )
...
def command_set_plan(args: argparse.Namespace) -> int:
    ...
    state["non_working_dates"] = list(
        dict.fromkeys(args.non_working_date)
    )
...
def command_assign_role(args: argparse.Namespace) -> int:
    ...
    state["role_assignments"][args.role] = require_string(
        args.assignee,
        "assignee",
    )
    write_json(Path(args.state), state)
...
def command_set_condition(args: argparse.Namespace) -> int:
    ...
    if args.condition not in definition["conditions"]:
        raise ReadinessError(
            f"condition_unknown:{args.condition}"
        )
    state["conditions"][args.condition] = (
        None
        if args.value == "unknown"
        else args.value == "true"
    )
    write_json(Path(args.state), state)
    return 0
...
def command_record(args: argparse.Namespace) -> int:
    ...
    state = load_json(Path(args.state))
    item_ids = {
        item["id"] for item in normalized_items(definition)
    }
    ...
    value["evidence"].extend(
        parse_evidence(entry) for entry in args.evidence
    )
...
def command_set_milestone(args: argparse.Namespace) -> int:
    ...
    if args.milestone not in state["milestones"]:
        raise ReadinessError(
            f"milestone_unknown:{args.milestone}"
        )
    if args.value != "clear":
        parse_datetime(args.value, "milestone")
    state["milestones"][args.milestone] = (
        None if args.value == "clear" else args.value
    )
    write_json(Path(args.state), state)
    return 0
...
def command_approve_gate(args: argparse.Namespace) -> int:
    ...
    pending_reason = f"gate_approval_pending:{args.gate}"
    core_incomplete = [
        reason
    ...
    if result.failures or core_incomplete:
        raise ReadinessError(
            f"gate_not_approvable:{args.gate}"
        )
    state["gate_approvals"][args.gate] = {
        "decision": "approved",
        "approver": require_string(
            args.approver,
            "approver",
        ),
        "decided_at": now_iso(),
...
def build_parser() -> argparse.ArgumentParser:
    root = argparse.ArgumentParser(
        description="Steam Store Readiness台帳を検証・更新する"
    )
    ...
    plan = subcommands.add_parser("set-plan")
    ...
    role = subcommands.add_parser("assign-role")
    common(role)
    ...
    condition = subcommands.add_parser("set-condition")
    common(condition)
    ...
    milestone = subcommands.add_parser("set-milestone")
    ...
    approve = subcommands.add_parser("approve-gate")
    common(approve)
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        print(
            f"Steam Store Readiness検証エラー: {exc}",
            file=sys.stderr,
        )
